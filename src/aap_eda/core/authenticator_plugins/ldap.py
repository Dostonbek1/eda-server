...
from ansible_base.authentication.common import (
    check_user_attribute_map,
    get_or_create_authenticator_user,
    update_user_claims,
)
from ansible_base.authenticator_plugins.base import (
...
from django_auth_ldap.backend import (
    LDAPBackend,
    LDAPSettings as BaseLDAPSettings,
)
...
class LDAPSettings(BaseLDAPSettings):
    ...
    defaults = dict(
        list(BaseLDAPSettings.defaults.items())
        + list({"GROUP_TYPE_PARAMS": {}}.items())
    )
    ...
    def __init__(self, prefix: str = "AUTH_LDAP_", defaults: dict = {}):
        ...
        # Check BindDN
        self.validate_ldap_dn(
            defaults.get("BIND_DN", None),
            error_entry_label="BIND_DN",
            with_user=False,
            required=False,
        )

        if (
            "BIND_PASSWORD" in defaults
            and type(defaults["BIND_PASSWORD"]) is not str
        ):
        ...
        self.validate_connection_options(
            defaults.get("CONNECTION_OPTIONS", None)
        )

        # Ensure GROUP_TYPE is a valid option and that GROUP_TYPE_PARAMS matches it
        self.validate_group_type(
            defaults.get("GROUP_TYPE", None),
            defaults.get("GROUP_TYPE_PARAMS", None),
        )
        ...
        for field, search_must_have_user in [
            ("GROUP_SEARCH", False),
            ("USER_SEARCH", True),
        ]:
            self.validate_ldap_search_field(
                field, defaults.get(field, None), search_must_have_user
            )
        ...
        if "SERVER_URI" in defaults:
            ...
            self.set_error(
                "SERVER_URI", "Must be a list of valid LDAP URLs", True
            )
        ...
        self.validate_ldap_dn(
            defaults.get("USER_DN_TEMPLATE", None),
            error_entry_label="USER_DN_TEMPLATE",
            with_user=True,
            required=True,
        )

        if "USER_ATTR_MAP" in defaults:
            for key, value in check_user_attribute_map(
                defaults["USER_ATTR_MAP"]
            ).items():
        ...
        for option in defaults.keys():
            if option not in LDAPSettings.defaults:
                self.set_error(
                    option,
                    "Is not a valid setting for an LDAP authenticator",
                    False,
                )

    def set_error(
        self, option_name: str, message: str, fail_configuration: bool
    ) -> None:
    ...
    def validate_connection_options(self, connection_options: Any) -> None:
        ...
        if type(connection_options) is not dict:
            self.set_error(
                "CONNECTION_OPTIONS", "Must be a dict of options", True
            )
            return
        ...
        for key in connection_options:
            if key not in valid_options:
                self.set_error(
                    f"CONNECTION_OPTIONS.{key}",
                    "Not a valid connection option",
                    True,
                )
            else:
    ...
    def validate_group_type(
        self, group_type_class_name: Any, group_type_params: Any
    ) -> None:
        ...
        if not group_type_class:
            self.set_error(
                "GROUP_TYPE", "Specified group type is invalid", True
            )
            return
        ...
        if missing_keys:
            ...
            for key in missing_keys:
                self.set_error(
                    f"GROUP_TYPE_PARAMS.{key}",
                    "Missing required field for GROUP_TYPE",
                    True,
                )
    ...
    def validate_ldap_search_field(
        self, search_field: str, data: Any, search_must_have_user: bool
    ) -> None:
        LIST_MESSAGE = (
        ...
        config_good = True
        if not self.validate_ldap_dn(
            data[0],
            error_entry_label=f"{search_field}.0",
            with_user=False,
            required=True,
        ):
            config_good = False
        ...
        if (
            type(data[1]) is not str
            or not data[1].startswith("SCOPE_")
            or not getattr(ldap, data[1], None)
        ):
            self.set_error(
                f"{search_field}.1",
                "Must be a string representing an LDAP scope object",
                False,
            )
            config_good = False

        if not self.validate_ldap_filter(
            data[2], f"{search_field}.2", with_user=search_must_have_user
        ):
            config_good = False
        ...
        if config_good:
            try:
                ...
                search_object = config.LDAPSearch(
                    data[0], getattr(ldap, data[1]), data[2]
                )
                setattr(self, search_field, search_object)
            except Exception as e:
                self.set_error(
                    search_field,
                    f"Failed to instantiate LDAPSearch object: {e}",
                    True,
                )

    def validate_ldap_filter(
        self, value: Any, error_entry_label: str, with_user: bool = False
    ) -> bool:
        # True = valid filter
        ...
        if with_user:
            if user_search_string not in value:
                self.set_error(
                    error_entry_label,
                    _(
                        'DN must include "{}" placeholder for username: {}'
                    ).format(user_search_string, value),
                    False,
                )
                return False
        ...
        if re.match(r"^\([A-Za-z0-9-]+?=[^()]+?\)$", dn_value):
            ...
            for sub_filter in dn_value[3:-2].split(")("):
                ...
                sub_filter_valid = self.validate_ldap_filter(
                    f"({sub_filter})", error_entry_label, with_user=False
                )
                if not sub_filter_valid:
                    return False
            return True
        self.set_error(
            error_entry_label, _("Invalid filter: %s") % value, False
        )
        return False
    ...
    def validate_ldap_dn(
        ...
        try:
            ...
            self.set_error(
                error_entry_label, _("Invalid DN: %s") % value, True
            )
            return False
...
class AuthenticatorPlugin(LDAPBackend, AbstractAuthenticatorPlugin):
    def __init__(self, database_instance=None, *args, **kwargs):
        ...
        if database_instance:
            self.settings = LDAPSettings(
                defaults=database_instance.configuration
            )
    ...
    def authenticate(
        ...
        if not self.database_instance.enabled:
            logger.info(
                f"LDAP authenticator {self.database_instance.name} is disabled, skipping"
            )
            return None
        ...
        if not self.settings.configuration_valid:
            logger.error(
                f"LDAP authenticator {self.database_instance.name} can not be used due to configuration errors."
            )
            return None

        if (
            self.settings.START_TLS
            and ldap.OPT_X_TLS_REQUIRE_CERT in self.settings.CONNECTION_OPTIONS
        ):
        ...
        try:
            ...
            if user_from_ldap is not None and user_from_ldap.ldap_user:
                users_groups = list(
                    user_from_ldap.ldap_user._get_groups().get_group_dns()
                )
                ...
                if user_from_ldap.ldap_user._connection_bound:
                    logger.debug(
                        f"Forcing LDAP connection to close for {self.database_instance.name}"
                    )
                    try:
            ...
            return update_user_claims(
                user_from_ldap, self.database_instance, users_groups
            )
        except Exception:
            logger.exception(
                f"Encountered an error authenticating to LDAP {self.database_instance.name}"
            )
            return None
    ...
    def process_login_messages(self, ldap_user, username: str) -> None:
        if ldap_user is None:
            logger.info(
                f"User {username} could not be authenticated by LDAP {self.database_instance.name}"
            )
            ...
            logger.info(
                f"User {username} authenticated by LDAP {self.database_instance.name}"
            )
    ...
    def validate_configuration(self, data: dict, instance: object) -> None:
        ...
        for field in ["USER_FLAGS_BY_GROUP", "DENY_GROUP", "REQUIRE_GROUP"]:
            if field in data:
                self.warnings[
                    field
                ] = "It would be better to use the authenticator field instead of setting this field in the LDAP adapter"

    def update_settings(self, database_authenticator: Authenticator) -> None:
        self.settings = LDAPSettings(
            defaults=database_authenticator.configuration
        )
